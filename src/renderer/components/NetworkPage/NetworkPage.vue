<template>
  <div id="main">
    <Checkbox-group v-model="tableColumnsChecked" @on-change="changeTableColumns">
      ...
      <Checkbox label="type">网络类型</Checkbox>
      <Checkbox label="vlan_id">VLanID</Checkbox>
...
<script>
  export default {
    data () {
      return {
        tableColumnsChecked: ['selection', 'name', 'status', 'type', 'vlan_id', 'action'],
        tableColumns: [],
        tableData: [],
        pageSize: 10
      }
    ...
    methods: {
      ...
      mockTableData () {
        ...
        for (let i = 0; i < this.pageSize; i++) {
          data.push({
            name: '网络' + Math.floor(Math.random() * 100 + 1),
            status: Math.floor(Math.random() * 3 + 1),
            type: ['external', 'a'][Math.floor(Math.random() * 2)],
            vlan_id: this.randomString(5)
          })
        }
        this.tableData = data
      },
